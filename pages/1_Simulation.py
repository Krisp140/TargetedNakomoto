...
with col1:
    ...
    if st.button("Run Simulation", key="run_standard_simulation"):
        ...
        ax1.plot(hashrate[:time_steps], 
                ...
                linestyle='--',
                color='red')
        ax1.set_title('Adjusted vs Baseline Hashrate (1-1-2022 to 3-8-2025)')
        ...
        # Plot block rewards
        ax2.plot(results['block_reward'], label='Adjusted Block Reward', color='orange')
        ...
        nodes = {
            ...
            'block_reward': (0.5, 0.8),   # Adjusted Block Reward (swapped with hashrate)
            'hashrate': (0.5, 0.2),       # Hashrate (swapped with block_reward)
        }
        ...
        connection_points = {
            ...
            'block_to_hash': [nodes['block_reward'][0], nodes['block_reward'][1] - node_height/2,  # bottom of block_reward
                             nodes['hashrate'][0], nodes['hashrate'][1] + node_height/2],  # top of hashrate
            'control_to_block': [nodes['control_params'][0] + node_width/2, nodes['control_params'][1],  # right of control_params
                               nodes['block_reward'][0] - node_width/2, nodes['block_reward'][1]],  # left of block_reward
            'hash_to_block': [nodes['hashrate'][0], nodes['hashrate'][1] + node_height/2,  # top of hashrate
                             nodes['block_reward'][0], nodes['block_reward'][1] - node_height/2],  # bottom of block_reward
        }
        ...
        # Direct arrow from Control Parameters to Block Reward
        flow_ax.annotate('', 
                        xy=(connection_points['control_to_block'][2], connection_points['control_to_block'][3]),  # target point
                        xytext=(connection_points['control_to_block'][0], connection_points['control_to_block'][1]),  # source point
